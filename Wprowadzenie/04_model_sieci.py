...
model = tf.keras.Sequential([
    ...
    tf.keras.layers.Dense(20, activation='sigmoid', kernel_regularizer=l2(0.01)),  # Pierwsza warstwa ukryta (liniowa aktywacja)
    tf.keras.layers.Dropout(0.2),  # 20% neuronów losowo wyłączanych

    tf.keras.layers.Dense(37, activation='relu', kernel_regularizer=l2(0.01)),    # Druga warstwa ukryta (ReLU)
    tf.keras.layers.Dropout(0.2),  # 20% neuronów losowo wyłączanych
